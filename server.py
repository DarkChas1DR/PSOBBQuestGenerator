...
def generate(prompt,current,model):
    ...
    payload={'model':model,'stream':False,'format':'json','messages':[{'role':'system','content':rules},{'role':'user','content':json.dumps({'request':prompt,'current_plan':current,'reference':REF})}]}
    ...
    return validate(json.loads(data['message']['content']))
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.split('?')[0] not in ['/', '/index.html','/app.js','/style.css','/data/reference.json']:return self.send_json(404,{'error':'Not found'})
        super().do_GET()
    def do_POST(self):
        if not self.local():return self.send_json(403,{'error':'Local access only'})
        ...
        try:
            ...
            data=json.loads(self.rfile.read(n));p=generate(data.get('prompt'),data.get('plan'),data.get('model'));self.send_json(200,{'plan':p})
